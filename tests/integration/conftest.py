...
def signing_app(tmp_path_factory):
    """
    Creates a single, fully configured application instance for the
    signing_service functional test session.
    """
    tmp_path = tmp_path_factory.mktemp("signing_functional_pki")
    ...
    passphrase = "functional-test-password"
    ...
    app.config.update({
        ...
        "SIGNING_SERVICE_API_SECRET": "functional-test-api-secret"
    })
    
    yield app

@pytest.fixture(scope='session')
def live_server(signing_app):
    """Starts a live server for the signing application."""
    server = LiveServer(app=signing_app, host='127.0.0.1', port=find_free_port(), wait=5)
    server.start()
    print(server.port)
    yield server
    server.stop()
